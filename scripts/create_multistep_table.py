...
def main():
    table = {}
    for log_dir in sorted(glob.glob("extra/multistep/*")):
        base = log_dir.split('/')[-1]
        splits = base.split('_')

        algo = "_".join(splits[:3])
        env = splits[3].split('-')[0]
        dataset = '-'.join(splits[3].split('-')[1:])
    ...
    with open("multistep_table.csv", "w") as f:
        writer = csv.writer(f)
